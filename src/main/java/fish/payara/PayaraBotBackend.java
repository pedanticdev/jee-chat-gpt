...
public class PayaraBotBackend {
	...
	private void contactUpdateCallback(Update update) {
		if (update.message() == null && update.callbackQuery() != null
			...
			SendMessage request = new SendMessage(update.callbackQuery().maybeInaccessibleMessage().chat().id(),
					"We'll be more than happy to hear from you. Don't have anything to say? Just say hi \uD83D\uDE42")
					.parseMode(ParseMode.HTML)
					.disableWebPagePreview(true)
					.disableNotification(true)
					.replyMarkup(inlineKeyboard);
	...
	private void serverUpdateCallback(Update update) {
		if (update.message() == null && update.callbackQuery() != null
			...
			SendMessage request = new SendMessage(update.callbackQuery().maybeInaccessibleMessage().chat().id(),
					"Payara Server Enterprise is our fully supported, production ready Jakarta EE and MicroProfile runtime for deploying modern applications.")
					.parseMode(ParseMode.HTML)
					.disableWebPagePreview(true)
					.disableNotification(true)
					.replyMarkup(inlineKeyboard);
