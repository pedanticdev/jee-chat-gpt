...
""";
    ...
    @Inject private CacheController cacheController;
    ...
    public PointsOfInterestResponse suggestPointsOfInterest(
            final String city, final BigDecimal budget) {
        ...
        try {
            String request =
                    String.format(
                            Locale.ENGLISH,
                            "I want to visit %s and have a budget of %,.2f dollars",
                            city,
                            budget);
    ...
    private String sendMessage(final String message) {
        ...
        log.log(
                Level.INFO,
                "Calling open AI service with the query " + chatCompletionRequest.toString());
    ...
    public RecipeSuggestion requestRecipe(final String recipe) {
        ...
        try (final Jsonb jsonb = JsonbBuilder.newBuilder().build()) {
            ChatCompletionRequest recipeRequest =
                    ChatCompletionRequest.builder()
                            .model(gptModel)
                            .temperature(0.8)
                            .messages(
            ...
            response.getChoices()
                    .forEach(choice -> recipeString.append(choice.getMessage().getContent()));
